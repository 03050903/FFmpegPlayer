...
{
    {
        ...
        while (m_ffmpeg->m_audioPacketsQueue.size() == 0)
        {
    ...
    for (;;)
    {
        if (m_ffmpeg->m_isPaused && !m_ffmpeg->m_isAudioSeekingWhilePaused)
        {
            ...
            if (boost::this_thread::interruption_requested())
            {
                ...
                CHANNEL_LOG(ffmpeg_threads) << "Audio thread broken";
                return;
            }
        ...
        for (;;)
        {
            ...
            if (!handlePacket(packet, resampleBuffer))
            {
                av_free_packet(&packet);
                break;
            }
...
bool AudioParseRunnable::handlePacket(AVPacket& packet, std::vector<uint8_t>& resampleBuffer)
{
    while (packet.size > 0)
    {
        int audioDecoded(0);
